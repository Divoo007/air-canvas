...
# Setup process: Install brew, and run brew install opencv
# next, run pip install numpy and pip install opencv-python
# then, go to you terminal, the directory of the python file and run "python3 air-canvas.py"
def setValues(x):
    print("")


# Hue color is set to blue. Get a Blue point and get to painting. Suggested usage: Camlin Dark Blue Brush Pen.
# Hue color's can be changed according to specification below:
...
cv2.createTrackbar("Lower Value", "Color detectors",
				78, 255, setValues)
# Lower Saturation: 72     Lower Value: 49
# Black Hue color: H - 0,255  S - 0,255  V - 0
